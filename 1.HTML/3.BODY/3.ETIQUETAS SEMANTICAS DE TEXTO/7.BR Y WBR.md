Los denominados **saltos de línea** son comportamientos en los que se salta a la siguiente linea para emular el comportamiento de pulsar  en un documento web. Este comportamiento, en Windows, se suele representar con los carácteres especiales `\r\n`, donde el `\r` es un **retorno de carro** y el `\n` es un **nueva línea**.

Estos comportamientos provienen de las antiguas **máquinas de escribir**, donde cuando terminabas de escribir una línea, tenías que volver al inicio de la misma (_retorno de carro_) y luego avanzar a la siguiente línea para seguir escribiendo y no escribir encima de lo que ya habías escrito.

### [La etiqueta `<br>`](https://lenguajehtml.com/html/texto/etiqueta-html-br-wbr/#la-etiqueta-br)

La etiqueta `<br>` representa el mencionado salto de línea, para avanzar a la línea siguiente y continuar escribiendo en una nueva línea, pero llevado a un documento web, ya que un  no realizará esa acción en el documento renderizado en el navegador.
...
#### [Uso de `<br>` para maquetar](https://lenguajehtml.com/html/texto/etiqueta-html-br-wbr/#uso-de-br-para-maquetar)
...
<br>
<br>
<p>Esta línea ahora está colocada donde debe estar.</p>
```
...
### [La etiqueta `<wbr>`](https://lenguajehtml.com/html/texto/etiqueta-html-br-wbr/#la-etiqueta-wbr)

Por otro lado, existe una etiqueta HTML menos conocida: la etiqueta `<wbr>`. Esta etiqueta se denomina **oportunidad de salto de línea** (_o división silábica_), y funciona de forma muy similar a `<br>`, sólo que en este caso el salto de línea no se realiza de forma obligatoria.
...
```html
<p>Super<wbr>cali<wbr>fragilístico<wbr>espialidoso.</p>
```
